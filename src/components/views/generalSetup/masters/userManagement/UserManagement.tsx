...
import { ActionBtns, ActiveUserLink, Btns, BtnsContainer, CardContainer, Fullname, Heading, LeftContainer, MainContainer, MainHeadingCont, RightContainer, RightSubContainer, RoleName, SearchContainer, UserDataRaw, Username } from './UserManagement.styles';
...
export default function UserManagement() {
    ...
    useEffect(() => {
        const getData = async () => {
            try {
                if (searchValue.trim() !== '') {
                    ...
                    const res: any = await GetSearchedUsersSer({ searchText: searchValue, currUserType: currUserType });
                    setFilteredUsers(res?.data); // Show filtered data
                } else {
                    // Call the GetUserByStatus API when searchValue is empty
        ...
        getData();
    }, [reloadData, searchValue, currUserType])
    ...
    const handleSearchChange = async (e: any) => {
        ...
        if (searchText.trim() === '') {
            ...
            try {
                const res: any = await GetSearchedUsersSer({ searchText: searchText, currUserType: currUserType });
                setFilteredUsers(res?.data); // Show filtered data
            } catch (err: any) {
                        ...
                        filteredUsers?.map((item: any, index: any) => (
                            <Paper elevation={5}>
                                <CardContainer key={index}>
                                    ...
                                    <UserDataRaw>
                                        <RoleName>{item.role_name}</RoleName>
                                        <Fullname>{item.uniqueEmpID}</Fullname>
                                    </UserDataRaw>
