                                ...
                                {filteredUserRoles?.map((row: any, index: any) => (
                                    ...
                                    >
                                        ...
                                        <TableCellData style={{ minWidth: '50px', padding: '2px 20px' }} align="center">
                                            <div style={{ padding: '5px', backgroundColor: '#840e779e', borderRadius: '5px', color: 'white', cursor: 'pointer' }} onClick={() => handleEditRole(row.userId)}>Edit</div>
                                            {
                                                showModal && <EditUserIpModal id={row?.id} userId={row?.userId} selectedRawId={selectedRawId} userData={{ username: row.username, department_Name: row.department_Name, iP_Address: row.iP_Address, status: row.status, ward_name: row.ward_name }} showModal={showModal} setShowModal={setShowModal} setReloadData={setReloadData} />
                                            }
                                        </TableCellData>
                                    </TableRow>)
                                ))}
