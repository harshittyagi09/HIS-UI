...
import { MainHeading, StyledLabel, RadioTd, LoginBtn, RadioBtn, LinkDiv, StyledTextField } from './InvestigationModal.styles';
import SelectDropdown from '../../common/select/SelectDropdown';
import { GetDoctors, GetPrevInvSer, InvOrderSer, ProOrderSer } from '../../../services/dropdownService/eOpdModalServices/investigationModalServices/InvestigationModalServices';
...
export const InvTable = (props: any) => {
  ...
  function createData(
    ...
    return { index, service_name, service_price, quantity, doctor_name, order_date, remarks };
  }

  const handleChange = (e: any, index: number) => {
    const { value } = e.target;
    props.updateQuantity(index, parseInt(value));
  }
      ...
      <Table sx={{ minWidth: 650 }} stickyHeader aria-label="simple table">
        <TableHead>
          <TableRow>
            ...
            <StyledTableCell style={{ minWidth: '100px' }} align="right">Service Price</StyledTableCell>
            <StyledTableCell style={{ minWidth: '100px' }} align="right">Quantity</StyledTableCell>
        ...
        <TableBody>
          {(gridData.length > 0) && gridData.map((row: any, index: any) => (
            ...
            >
              <StyledTableCell style={{ fontSize: '13px', padding: '10px' }} component="th" scope="row">{row.service_name}</StyledTableCell>
              <StyledTableCell style={{ fontSize: '13px', padding: '10px' }} align="center">{row.service_price}</StyledTableCell>
              <StyledTableCell style={{ fontSize: '13px', padding: '10px' }} align="center">
                <StyledTextField type='number' name="Qty" value={row.quantity} onChange={(e: any) => handleChange(e, index)} />
              </StyledTableCell>
              <StyledTableCell style={{ fontSize: '13px', padding: '10px' }} align="center">{row.doctor_name}</StyledTableCell>
              <StyledTableCell style={{ fontSize: '13px', padding: '10px' }} align="center">{row.order_date}</StyledTableCell>
              <StyledTableCell style={{ fontSize: '13px', padding: '10px' }} align="right">{row.remarks}</StyledTableCell>
              <StyledTableCell style={{ fontSize: '13px', padding: '10px' }} align="right">
                <div onClick={() => props.handleRemoveClick(index)} style={{ padding: '5px 10px', margin: '2px', borderRadius: '5px', backgroundColor: '#840e779e', color: 'white', fontWeight: '500', cursor: 'pointer', fontSize: '13px' }}>Remove</div>
              </StyledTableCell>
            </TableRow>
          ))}
        </TableBody>
...
function InvestigationModal(props: any) {
  ...
  //         window.alert("Please select any service from dropdown");
  //       } else {
  ...
  const handleOrderServicesClick = (e: any) => {
    ...
    if (selectedService && selectedService.serviceName) {
      setGridObjects((prevGridObjects: any) => {
        ...
        if (isServiceAlreadyAdded) {
          window.alert("This service is already added!");
          return prevGridObjects;
        }
      ...
      window.alert("Please select a service from the dropdown.");
    }
  }
  ...
  const handlePrevDiagClick = async (e: any, deptId: any, patientId: any, stageId: any, opdNo: any) => {
    try {
      const res: any = await GetPrevInvSer(deptId, patientId, stageId, opdNo);
      if (res?.status === 200) {
        res?.data?.map((item: any, index: any) => {
          ...
          setGridObjects((prevGridObjects: any) => {
            const isServiceAlreadyAdded = prevGridObjects.some(
              (obj: any) => obj.serviceName === newSelectedServiceData.serviceName
            );
            ...
            return [...prevGridObjects, newSelectedServiceData];
          });
        })
      }
    } catch (err: any) {
      console.log("GetPrevInvSer api err", err);
    }
  }

  const updateQuantity = (index: number, quantity: number) => {
    const updatedGridObjects = [...gridObjects];
    updatedGridObjects[index].quantity = quantity;
    setGridObjects(updatedGridObjects);
  }
    ...
    <>
      ...
      <div style={{ border: '0px solid green', padding: '10px 20px', minWidth: '70%', backgroundColor: '#f5f5f5', boxShadow: '0px 1px 5px 0.25px #80808057' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          {/* <LoginBtn onClick={(e: any) => handlePrevDiagClick(e, rowData.deptId, rowData.patientId, 6)}>Previous Diagnosis</LoginBtn> */}
          <LinkDiv style={{ display: 'flex' }}>Want to see Previous OPD Diagnosis?<div onClick={(e: any) => handlePrevDiagClick(e, rowData.deptId, rowData.patientId, 6, rowData.opdNo)} style={{ color: '#4074c1', cursor: 'pointer', paddingLeft: '5px' }}>Click here</div></LinkDiv>
        </div>
        {
          gridObjects.length > 0 && (
            <>
              <InvTable gridObjects={gridObjects} handleRemoveClick={handleRemoveClick} updateQuantity={updateQuantity} />
              {
