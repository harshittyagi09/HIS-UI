...
import { MainHeading, StyledLabel, DropdownLabel, SubmitBtn, StyledTextField, LinkDiv } from './PharmaModal.styles';
import SelectDropdown from '../../common/select/SelectDropdown';
import { GetMedicinesSer, GetPrevPharmaSer } from '../../../services/dropdownService/eOpdModalServices/pharmaModalServices/PharmaModalServices';
...
function PharmaModal(props: any) {
    ...
    const handleAddMedicine = (e: any) => {
        ...
        if (pharmaData.item_name != '' && pharmaData.dose != "" && pharmaData.for_days != '') {
    ...
    const handlePrevDiagClick = async (e: any, deptId: any, patientId: any, stageId: any) => {
        try {
            const res: any = await GetPrevPharmaSer(deptId, patientId, stageId);
            if (res?.status === 200) {
                res?.data?.map((item: any, index: any) => {
                    const isSelectedDose: any = doses.find((dose: any) => dose.abbreviation === item.dose);
                    const selectedDoseQty = isSelectedDose && isSelectedDose.dose;
                    const newSelectedMedicine = {
                        item_name: item.itemName,
                        item_code: item.itemCode,
                        dose: item.dose,
                        doseVal: selectedDoseQty,
                        isUomIdmatched: uomIds?.some((item: any) => parseInt(item.uom_id) === item.uomId),
                        for_days: item.fordays,
                        Remarks: '',
                        service_unit_price: (item.itemName).split('$')[1].trim().split(' ')[0],
                    ...
                    setGridObjects((prevGridObjects: any) => {
                        const isServiceAlreadyAdded = prevGridObjects.some(
                            (obj: any) => obj.serviceName === newSelectedMedicine.item_name
                        );
                        ...
                        return [...prevGridObjects, newSelectedMedicine];
                    });
                })
            }
        } catch (err: any) {
            console.log("GetPrevPharmaSer api err", err);
        }
    };
        ...
        <>
            ...
            <div style={{ border: '0px solid green', padding: '10px 20px', minWidth: '70%', backgroundColor: '#f5f5f5', boxShadow: '0px 1px 5px 0.25px #80808057' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    {/* <SubmitBtn onClick={(e: any) => handlePrevDiagClick(e, rowData.deptId, rowData.patientId, 7)}>Previous Diagnosis</SubmitBtn> */}
                    <LinkDiv style={{ display: 'flex' }}>Want to see Previous OPD Diagnosis?<div onClick={(e: any) => handlePrevDiagClick(e, rowData.deptId, rowData.patientId, 7)} style={{ color: '#4074c1', cursor: 'pointer', paddingLeft: '5px' }}>Click here</div></LinkDiv>
                </div>
                {
