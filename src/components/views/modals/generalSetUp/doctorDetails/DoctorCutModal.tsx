...
import { GetDoctorCutDetailsSer, SetDoctorCut } from '../../../../../services/generalSetUpServices/doctorDetailsServices/DoctorDetailsServices';
...
export default function DoctorCutModal(props: any) {
    ...
    //     const getData = async () => {
    //         try {
    //             const res: any = await GetDoctorCutDetailsSer(doctorID);
    //             // setDoctorDetails(res?.data);
    //             // setReloadData(false);
    //             setDoctorCutData({
    //                 doctor_id: res?.data[0]?.doctor_id,
    //                 doctor_cut_per: res?.data[0]?.doctor_cut_per,
    //                 service_classification: res?.data[0]?.service_classification,
            ...
            try {
                ...
                if (res?.data && res?.data[0]) {
                    setDoctorCutData({
                        ...
                        doctor_cut_per: res?.data[0]?.doctor_cut_per,
                        service_classification: res?.data[0]?.service_classification,
                        effective_date: formatSelectedDate(res?.data[0]?.effective_date)
                    });
                    ...
                    <div style={{ margin: '15px' }}>
                        <DataContainer>
                            ...
                            <Container>
                                ...
                                <SelectDropdown required={true} dropdown='doctorCutSerType' doctor_cut_serType={doctorCutData.service_classification} values={doctorCutSerTypeOptions} handleServiceType={(e: any) => handleServiceType(e)} />
                            </Container>
                            <Container>
                                ...
                                <LocalizationProvider dateAdapter={AdapterDayjs}>
                                    <DatePicker
                                        ...
                                        label={doctorCutData.effective_date === '' ? "Choose Date" : doctorCutData.effective_date}
                                        format="DD/MM/YYYY"
