...
const OPDReg = (props: any) => {
    ...
    const [selectedDropdownValues, setSelectedDropdownValues] = useState({
        ...
        SpecID: 0,
        UnitId: 0,
        partnerId: props?.billingData?.partner_id === null ? 0 : props?.billingData?.partner_id,
        IsApplyEmergencyCharge: 'N',
        CampId: props?.billingData?.camp_id === null ? 0 : props?.billingData?.camp_id,
        OPDDiscountID: 0
    });
            ...
            if (loading) {
                ...
                if (res?.status === 200) {
                    ...
                    if (RefferedOPDInsertionServiceRes?.status === 200) {
                        ...
                        const isUseAuthorized: any = Object.values(props?.userMenu).some((value: any) => value?.uiRoute == '/eopd');
                        props?.setOpenRegModal(false);
                        // isUseAuthorized && history.push('/patientForRequisition/3')
                        isUseAuthorized && history.push('/eopd')
    ...
    return (
        <div>
            ...
            {
                showTable && <OPDDoctorScheduleTable setShowTable={setShowTable} selectedDropdownValues={selectedDropdownValues} patientId={props?.patientDetails?.patient?.patient_id} billingData={props?.billingData} />
            }
            {props?.page != 'RefferedPatient' && <FeeTable page={props?.page} selectedPatientType={selectedPatientType} patientDetails={props.patientDetails} billingData={props?.billingData} />}
        </div>
    )
}
