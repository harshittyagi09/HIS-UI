...
import { GetJRService, HandleOpdBillPrint, OPDBillLogInfoInsertionSer, OPDUserVerificationService, OpdFinalBillingService, PaymentModesService } from '../../../services/registrationService/opdRegistrationService/opdBillingServices/OpdBillingServices';
    ...
    const handleBillNowClick = async () => {
        ...
        try {
            ...
            if ((OPDUserVerificationServiceRes?.data?.flag === true) && ((props?.billingData?.JR_Id != 0))) {
                const res: any = props?.billingData?.pay_mode !== "" ? await OpdFinalBillingService({
                    billingData: props.billingData,
                    selectedDoctor: props.selectedDoctor,
                    flag: props.patientDetails.flag,
                    page: props?.page
                }) : window.alert("Please select paymode..");
                if (res?.status === 200) {
                    ...
                    try {
                        const opdBillInsertionSerRes: any = await OPDBillLogInfoInsertionSer(0, props?.billingData?.patient_id, receiptNo, billNo, 0, 'CA', '');
                        if (opdBillInsertionSerRes?.status === 200) {
                            // window.alert("Billing data log info inserted successfully :)");
                        } else {
                            window.alert("Some pblm occured in billing data insertion");
                        }
                    } catch (error: any) {
                        console.error('OPDBillLogInfoInsertionSer Error:', error);
                    }
                    props.setShowModal(false);
                    history.push('/registration');
                    try {
                        const billPrintingRes: any = await HandleOpdBillPrint('PRINTOPDBILL');
                        if (billPrintingRes?.data[0]?.keyValue === 'Y') {
                        ...
                        } else if (billPrintingRes?.data[0]?.keyValue === 'N') {
                            window.alert("OPD Bill Printing not activated");
                        }
                    } catch (error: any) {
                        console.error('HandleOpdBillPrint Error:', error);
                    }
                } else {
