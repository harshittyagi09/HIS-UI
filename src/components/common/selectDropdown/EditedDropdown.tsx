...
export const EditedDropdown = (props: any) => {
  const { dropdown, options, payMode } = props;
  const [selectedVal, setSelectedVal] = useState('');

  useEffect(() => {
    if (props.dropdown === 'updatePaymentMode') {
      setSelectedVal(payMode);
    }
  }, [props.dropdown]);
  ...
  const handleChange = (e: any) => {
    ...
    if (props.dropdown === 'resetPswrd') {
      ...
      props.handledeptsChange(e);
    } else if (props.dropdown === 'updatePaymentMode') {
      props.handleUpdatePaymode(e);
    } else if (props.dropdown === 'creditCardType') {
      props.handleCreditCardType(e);
    } else if (props.dropdown === 'refundReason') {
      props.handleRefundReason(e);
    }
  };
    ...
    >
      ...
      >
        {options?.map((data: any, index: number) => (
          dropdown === 'resetPswrd' && <MenuItem sx={{ fontSize: '13px' }} key={index} value={data.locationName}>{data.locationName}</MenuItem> ||
          dropdown === 'doctorTypes' && <MenuItem sx={{ fontSize: '13px' }} key={index} value={data.doctorType}>{data.doctorType}</MenuItem> ||
          dropdown === 'depts' && <MenuItem sx={{ fontSize: '13px' }} key={index} value={data.dept_name}>{data.dept_name}</MenuItem> || //updatePaymentMode
          dropdown === 'updatePaymentMode' && <MenuItem sx={{ fontSize: '13px' }} key={index} value={data.description}>{data.description}</MenuItem> || //creditCardType
          dropdown === 'creditCardType' && <MenuItem sx={{ fontSize: '13px' }} key={index} value={data}>{data}</MenuItem> ||
          dropdown === 'refundReason' && <MenuItem sx={{ fontSize: '13px' }} key={index} value={data.refund_reason}>{data.refund_reason}</MenuItem>
        ))}
      </Select>
    </FormControl>
  );
