...
const SelectDropdown = (props: any) => {
  ...
  const { dropdown, values, doctor_type, doctor_cut_serType } = props;
  ...
  useEffect(() => {
    if (props.dropdown === 'invesDoctor') {
      ...
      setSelectedVal(doctor_cut_serType);
    }
  }, [props.dropdown, doctor_cut_serType])
  ...
  const handleChange = (e: any) => {
    ...
    // TransactionsHandling (Selected days to see bills) dropdown
    else if (props.dropdown === 'selectDaysBill') {
      props.handleDays(e);
    }
  };
    ...
    >
      ...
      >
        {values?.map((data: any, index: any) => (
          ...
          // TransactionsHandling (Selected days to see bills) dropdown
          dropdown === 'selectDaysBill' && <MenuItem sx={{ fontSize: '13px' }} key={index} value={data.label}>{data.label}</MenuItem>
        ))}
      </Select>
    </FormControl>
  );
