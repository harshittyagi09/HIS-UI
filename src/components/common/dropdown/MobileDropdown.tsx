...
function MobileDropdown({ userModules, userMenu, setIsMenuClick }: any) {
    ...
    const handleRoleClick = (role: any) => {
        ...
        } else if (role === 'Payment Acknowledge') {
            history.push('/paymentAcknowledge');
        }
    }
    ...
    return (
        <div>
            <ModuleDropdownContainer>
                <div style={{ maxHeight: '300px', overflowY: 'auto', width: '20%'}}>
                ...
                <RoleContainer>
                    {role.map((ele, index) => (
                        <UserRole hoveredRole={hoveredRole === ele} key={index} onMouseEnter={() => handleRoleHover(ele)}>
                            {ele}
                            <ArrowForwardIosIcon style={{ color: 'white', padding: '0px 10px', fontSize: '10px' }} />
                        </UserRole>
                    ))}
                </RoleContainer>
                <RoleValuesContainer>
