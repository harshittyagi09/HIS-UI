...
export const GetPrevCCSer = async (deptId: any, patientId: any) => {

    try {
        const response = await axios.get(`http://localhost:34553/api/Accounts/GetPreviuosCCService?deptId=${deptId}&patientId=${patientId}`);
        return response;

    } catch (error: any) {
        console.error('GetPrevCCSer Error', error.response?.data || error.message);
    }
}
...
export const GetPrevProvDiagSer = async (deptId: any, patientId: any) => {

    try {
        const response = await axios.get(`http://localhost:34553/api/Accounts/GetPreviuosProvisionalDiagnosisService?deptId=${deptId}&patientId=${patientId}`);
        return response;
...
export const GetPrevTTSer = async (deptId: any, patientId: any) => {

    try {
        const response = await axios.get(`http://localhost:34553/api/Accounts/GetPreviuosTTService?deptId=${deptId}&patientId=${patientId}`);
        return response;

    } catch (error: any) {
        console.error('GetPrevTTSer Error', error.response?.data || error.message);
    }
}
