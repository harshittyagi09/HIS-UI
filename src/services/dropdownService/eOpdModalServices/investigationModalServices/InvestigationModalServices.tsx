...
export const GetPrevInvSer = async (deptId: any, patientId: any, stageId: any, OpdNumber: any) => {

    try {
        const response = await axios.get(`http://localhost:34553/api/Accounts/GetInvestgationService?patientId=${patientId}&deptId=${deptId}&stageId=${stageId}&OpdNumber=${OpdNumber}`);
        return response;

    } catch (error: any) {
        console.error('GetPrevInvSer Error', error.response?.data || error.message);
    }
}