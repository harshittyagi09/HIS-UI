...
// Search
// SearchText will be username || uniqueEmpID
export const GetSearchedUsersSer = async (props: any) => {
    try {
        const getSearchedUsersSerRes: any = await axios.get(`http://localhost:34553/api/GeneralSetup/GetUserDetails?SearchText=${props?.searchText}&status=${props?.currUserType}`);
...
export const GetHrmsUserDetails = async (empId: any) => {
    ...
    try {
        ...
        if (error?.response?.data?.message === 'Unique ID not found. Or In Active') {
            window.alert("Employee ID not found in HRMS...");
        }
