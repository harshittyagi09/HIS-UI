...
export const GetDepartments = async () => {
    try {
        const getDepartmentsRes: any = await axios.get('http://localhost:34553/api/Accounts/HiS_DEPARTMENT_Info?For=doctorDetails');
...
export const GetDoctorCutDetailsSer = async (doctorId: any) => {
    try {
        const response: any = await axios.get(`http://localhost:34553/api/GeneralSetup/GetDoctorCutCharge?DoctorId=${doctorId}`);
        return response;
    } catch (err: any) {
        console.error("GetDoctorCutDetailsSer error", err);
        throw err;
    }
}
